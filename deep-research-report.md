# Inspectable Symbolic Compression for Long-Context Reasoning: A Mixed-Results Study

## 장문 추론을 위한 이산 심볼 압축: 자연어 요약 대비 한계 규명 (mixed-results)

> **상태:** dev-scale 실험을 반영한 mixed-results 원고. §8의 일부 값은 dev pilot(소규모)이며 full
> registered scale은 후속이다. 본 논문은 새 압축기를 "성공적으로 제안"하지 않는다 — prompt-only
> 이산 심볼 압축이 자연어 요약 대비 효율적 압축기가 되지 못함을 실험적으로 규명하고, 그 한계와
> 향후 방향(programmatically executable semantic IR)을 정직하게 보고한다.

## 초록

대규모 언어 모델의 장문 추론 비용을 줄이기 위한 기존 연구는 원문에서 중요한 토큰을 선택하거나 문맥을 연속 벡터로 압축해 왔다. 본 논문은 긴 문서를 재사용 가능한 이산 텍스트 심볼과 짧은 사전으로 변환하는 **Inspectable Symbolic Compression (ISM)** 을 연구하고, 동일 모델·동일 예산에서 자연어 요약(Model Summary)을 강한 baseline으로 두고 정직하게 비교한다. ISM은 문서를 `Z1`, `Z2`, ... 형태의 심볼과 사전으로 표현하며, 사전 제거·오염·결론 반전·라벨 교환 같은 통제된 개입으로 표현의 기능을 검사할 수 있다.

우리는 네 가지 연구 질문을 평가하고 **혼합 결과**를 보고한다. (RQ1) 심볼 구조와 사전 의미내용은 실제로 추론에 사용된다 — dev-scale에서 결론 반전 corruption과 심볼 구조 제거가 각각 정확도를 유의하게 낮췄다(긍정). (RQ3) 그러나 동일 토큰 예산에서 Model Summary가 ISM을 정확도·효율 모두에서 유의하게 능가했다(부정). (RQ4) 문서당 1회 압축을 다수 질문에 재사용하는 비용-정확도 frontier에서도 Model Summary가 ISM을 지배했다(부정). 즉 **prompt-only ISM은 자연어 요약보다 효율적인 압축기가 아니다.** 동시에 우리는 초기의 "사전을 망가뜨려도 성능이 안 떨어진다"는 null 결과가 (i) 압축 purity 부족과 (ii) 내용 보존적 corruption 설계라는 측정 결함의 산물이었음을 진단·교정하는 과정을 함께 보고한다. 본 연구는 ISM을 더 나은 압축기로 주장하지 않고, prompt-only symbolic compression의 한계를 규명하며 향후 연구가 LLM이 다시 읽는 텍스트가 아니라 parser/checker/executor가 다루는 **실행 가능한 semantic IR**로 이동해야 함을 시사한다. (RQ2 라벨 교환은 후속.)
...
본 연구의 출발 가설은 ISM이 **경쟁적인 정보 보존 효율을 가지면서 동시에 조작 가능한** 압축 표현이 될 수 있다는 것이었다. 그러나 dev-scale 실험 결과 이 가설의 효율 부분은 지지되지 않았다(§8.3, §8.4): 동일 모델·동일 예산에서 자연어 요약이 ISM보다 정확하고 저렴했다. 따라서 본 논문은 새 압축기의 성공을 주장하는 대신, **prompt-only 이산 심볼 압축이 자연어 요약 대비 효율적 압축기가 되지 못하는 한계를 실험적으로 규명**하고, 그 원인(§의 진단)과 향후 방향을 보고하는 mixed-results 연구로 스스로를 자리매김한다.

**결과 요약(미리보기).** (1) ISM 내부 구조는 실제로 사용된다 — 사전 의미내용 반전과 심볼 구조 제거가 각각 정확도를 유의하게 낮춘다(RQ1, dev-scale). (2) 그러나 동일 예산에서 Model Summary가 ISM을 정확도·효율 모두에서 유의하게 능가한다(RQ3). (3) 재사용 비용-정확도에서도 Model Summary가 ISM을 지배한다(RQ4). (4) 초기의 Δmap≈0 null은 가설 반증이 아니라 압축 purity 부족 + 내용 보존적 corruption이라는 **측정 결함**의 산물이었고, 이를 교정한 뒤에야 RQ1 신호가 드러났다. 종합하면 ISM의 잠재적 가치는 "더 나은 압축"이 아니라 LLM 밖의 프로그램이 다룰 수 있는 **검사·변환·실행 가능한 semantic IR**에 있으며, 이는 본 prompt-only 설정의 범위를 넘는 후속 과제다.
...
> **결과 요약(dev-scale).** 6.1(§8.1): 교정 후 ISM의 심볼 구조·사전 의미내용이 기능적으로 사용됨(긍정).
> 6.3(§8.3): 동일 예산에서 Model Summary가 ISM을 유의하게 능가(부정, 부록 A 기준 #1 미충족).
> 6.4(§8.4): 재사용 비용-정확도에서도 Model Summary가 ISM을 지배(부정). 일부 표는 dev pilot 규모이며
> full registered scale은 후속이다. 값과 통계는 각주의 증거(`docs/evidence/`)에 원시 파일·해시로 보관한다.
...
#### 8.4.1 Reuse 비용-정확도 (dev pilot, 분석적)

> 문서당 1회 압축 표현을 n개 질문에 재사용할 때의 토큰 비용(§6.4 공식). 비용은 분석적이며 토큰
> 수(|x|=1348, |q|=5.5)는 long-context 데이터, |z|·정확도는 §8.3.2(budget 256)에서 가져왔다.
> 질문당 정확도는 n과 무관(같은 캐시가 모든 질문에 답)하므로 method-level 상수다. 증거:
> [docs/evidence/reuse-N40/](../docs/evidence/reuse-N40/README.md).

| 방법 | n=1 | n=8 | n=32 | n=64 (end-to-end / serving) | Accuracy |
|---|---:|---:|---:|---:|---:|
| Full Context | 1354 | 10828 | 43312 | 86624 / 86624 | 0.700 |
| Model Summary | 1526 | 2168 | 4369 | **7305 / 5870** | **0.787** |
| ISM | 1548 | 2265 | 4725 | 8005 / 6560 | 0.475 |
| Oracle Gold | 1530 | 2184 | 4428 | 7420 / 5984 | 0.512 |
| Keyword Extract | 1820 | 3492 | 9225 | 16868 / 15286 | 0.512 |

모든 압축 방법은 **n=2부터 Full Context보다 저렴**하며 n=64에서 약 12배 저렴하다(86624 → 7305).
다만 캐시 방법 사이에서는 Model Summary가 비용(토큰 86 vs ISM 97)과 정확도(0.787 vs 0.475)
모두에서 ISM을 **지배**한다.

보고 문장:

> Reuse는 압축 표현의 분명한 실용 이점을 보인다: 질문 2개부터 모든 압축 방법이 Full Context보다
> 저렴하고 질문 수가 늘수록 격차가 커진다. 그러나 이는 ISM 고유의 이점이 아니라 캐시 가능한 모든
> 표현의 공통 이점이며, 비용-정확도 frontier에서는 Model Summary가 ISM을 지배한다. 따라서 본
> 연구는 ISM을 **더 효율적인 캐시**가 아니라, Model Summary가 제공하지 못하는 **검사·개입 가능한
> 의미 캐시**로 자리매김한다(§9). 이 주장의 직접 근거는 후속 constructive intervention 실험이다.

#### 8.4.2 QASPER (실험 완료 후 채움)
...
본 논문은 긴 문서를 이산 텍스트 심볼과 사전으로 변환하는 Inspectable Symbolic Compression(ISM)을 연구하고, 동일 모델·동일 예산의 자연어 요약을 강한 baseline으로 두고 정직하게 평가했다. 결과는 **혼합적**이다.

**무엇이 작동했나.** 진단·교정 후, ISM의 내부 구조는 실제로 추론에 사용된다(RQ1, dev-scale N=240). 사전의 결론을 반전하면 정확도가 유의하게 하락했고(\(\Delta_{\mathrm{map}}^{\mathrm{flip}}\) p=0.032), 심볼 구조 제거는 무작위 대비 유의하게 더 나빴다(\(\Delta_{\mathrm{symbol}}\) p=5e-4). 반면 라벨 순열은 영향이 없었다 — 모델은 표면 라벨 암기가 아니라 의미내용과 관계 구조를 사용한다. 초기의 "사전을 망가뜨려도 성능이 안 떨어진다"는 null은 가설 반증이 아니라 압축 purity 부족과 내용 보존적 corruption이라는 측정 결함이었음을 우리는 분해해 보였다.

**무엇이 작동하지 않았나.** 그러나 출발 가설의 핵심인 "효율적 압축기"는 지지되지 않았다. 동일 예산에서 Model Summary가 ISM을 같은 문항 paired 비교로 모든 예산에서 유의하게 능가했고(RQ3, p<0.01; 부록 A 기준 #1 미충족), 문서당 1회 압축을 재사용하는 비용-정확도 frontier에서도 Model Summary가 ISM을 비용·정확도 모두에서 지배했다(RQ4). 요약과 ISM은 모두 "LLM이 생성하고 LLM이 다시 읽는 텍스트"라는 점에서 같은 범주이며, 그 범주 안에서는 더 짧고 정확한 자연어 요약이 더 강한 baseline이다.

**시사점과 향후 방향.** 따라서 prompt-only symbolic compression을 더 나은 압축기로 제시하는 길은 본 결과로 닫힌다. 남는 가능성은 ISM을 **LLM이 다시 읽는 텍스트가 아니라, parser/checker/executor가 다루는 실행 가능한 semantic IR**로 취급하는 것이다(부록 B). 이 경로에서 ISM의 차별점은 "수정 가능"(요약도 수동 수정은 가능)이 아니라, **스키마 기반의 자동·국소·검증 가능한 변환과 LLM 없는 symbolic execution**이다. 본 prompt-only 연구는 이 전환의 필요성을 음의/혼합 결과로 규명한 것이며, 그 검증은 후속 과제로 남긴다. RQ2(미관측 라벨 교환)와 full registered-scale 확정도 후속이다.
...
**원시 증거 상태.** 본 연구의 dev-scale 결과는 코드 저장소의 [`docs/evidence/`](../docs/evidence/)에
실행별로 정리되어 있다(각 디렉터리에 산출물 원본 또는 sha256, `environment.json`(commit·GPU·패키지
버전), 재현 명령 포함). 핵심 실행:
- 6.1 dev scale-up: `docs/evidence/ablation-qwen7b-N120/` (config_hash `c907f84…`, commit `87e216e`)
- 6.3 fixed-budget pilot: `docs/evidence/fixed-budget-N40/` (commit `5d4500d`)
- 6.4 reuse: `docs/evidence/reuse-N40/` · filler sanity: `docs/evidence/fixed-budget-filler-sanity/`
- 진단 경과: [`docs/reviews/llm-ism-diagnostic.md`](../docs/reviews/llm-ism-diagnostic.md), 결정 기록
  [`docs/decisions/`](../docs/decisions/) (ADR 0001 config_hash, 0002 QASPER 채점)

대용량 예측 파일(predictions.jsonl 등)은 GPU 실행 환경에 보관하고 sha256만 기록했으며, 명시된 commit과
config로 재생성된다. 일부 표는 dev pilot 규모이므로 full registered scale 수치로 인용하지 않는다.
...
| 사전등록 기준 | 판정 (dev-scale) | 근거 |
|---|---|---|
| #1 ISM이 Model Summary보다 유의하게 높은 AR 또는 ES | **실패** | §8.3.2: paired Model Summary − ISM = +0.25~+0.375, McNemar p<0.01 (모든 예산). §8.4: 재사용 frontier에서도 summary가 ISM 지배 |
| #2 Symbol Only > Random **이고** Full+Dict > (amended) Flipped | **충족** (dev-scale) | §8.1.2: \(\Delta_{\mathrm{symbol}}\)=+0.108 (p=5e-4), \(\Delta_{\mathrm{map}}^{\mathrm{flip}}\)=+0.079 (p=0.032) |
| #3 Unseen Swap + Dict가 Original 성능 대부분 유지 | **미검증** | RQ2(6.2 Swap, LoRA) 미실행 — 후속 |

부록 A 본문 규칙대로 **#1이 실패했으므로 ISM의 효율 우위(더 나은 압축기)는 주장하지 않는다.** #2는
ISM 내부 구조가 기능적으로 사용됨을 지지하나, 이는 효율 우위와 별개의 진술이다. 최종 확정은 full
registered scale에서 수행한다.

## 부록 B. 후속 연구

**주 방향 — 실행 가능한 semantic IR.** 본 연구의 음의/혼합 결과는 핵심 후속 방향을 가리킨다. 현재
ISM은 "LLM이 생성하고 LLM이 다시 읽는 텍스트"라 자연어 요약과 같은 범주이며, 그 범주에서는 요약이
더 짧고 정확하다(§8.3, §8.4). ISM이 요약과 본질적으로 달라지려면 LLM이 아니라 **LLM 밖의 프로그램**이
표현을 다뤄야 한다.

| 항목 | Model Summary | ISM as semantic IR (제안) |
|---|---|---|
| 생성 | LLM | LLM이 문서를 IR로 **컴파일** |
| 형식 | 자유 텍스트 | schema (parser 가능) |
| 검증 | 어려움 | validator로 규칙 누락·일관성 검사 |
| 편집 | 수동 rewrite | rule_id 기반 **국소·자동·검증 가능** 변환 |
| 실행 | LLM 필요 | symbolic executor가 **LLM 없이** 답 계산 가능 |

따라서 구체적 후속 실험은 다음과 같다.

1. **Compile-and-execute:** LLM이 생성한 ISM을 parser로 AST화하고, Synthetic Rule-QA의 gold rule
   graph 의미론으로 **symbolic executor**가 LLM QA 없이 답을 계산한다. 핵심 질문은 "LLM이 긴 문서를
   *실행 가능한* IR로 컴파일할 수 있는가"이며, 요약은 executor가 없어 직접 비교 불가(또는 LLM parser
   필요)라는 점에서 비로소 범주가 갈린다.
2. **Programmatic editability:** corrupted/flipped ISM을 rule_id 기반 deterministic edit으로 수정한 뒤
   schema validator 통과·관련 질문만 회복·무관 질문 유지를 측정한다. 지표는 정확도뿐 아니라
   `repair_success`, `targeted_effect_rate`, `collateral_damage_rate`, `parse_success`,
   `tokens_changed`, `needs_llm_rewrite`로 하여 요약의 수동 rewrite와 **편집의 locality·검증
   가능성**을 대비한다. 주장은 "요약은 수정 불가"가 아니라 "ISM은 구조화 연산으로 자동·국소·검증 가능한
   수정을 제공한다"이다.
3. RQ2 미관측 라벨 교환(소규모 LoRA), full registered scale 확정, QASPER answer-F1 평가.

기타 범위 외 방향: Symbol + Residual Memory, adaptive symbolization, continuous-discrete hybrid
compression, embedding slot, RAG 및 멀티턴 대화 적용.
